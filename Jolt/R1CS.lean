...
structure JoltR1CSInputs (f : Type) : Type where
  ...
  NextPC : ZKExpr f
  NextIsVirtual : ZKExpr f
  NextIsFirstInSequence : ZKExpr f
...
instance: Witnessable f (JoltR1CSInputs f) where
  witness := do
    ...
    let NextIsVirtual <- Witnessable.witness
    let NextIsFirstInSequence <- Witnessable.witness
    ...
    let OpFlags_VirtualInstruction <- Witnessable.witness
    ...
    let OpFlags_IsFirstInSequence <- Witnessable.witness
    ...
    pure {
      ...
      NextPC := NextPC
      NextIsVirtual := NextIsVirtual
      NextIsFirstInSequence := NextIsFirstInSequence
      ...
      OpFlags_VirtualInstruction := OpFlags_VirtualInstruction
      ...
      OpFlags_IsFirstInSequence := OpFlags_IsFirstInSequence
    }
...
def uniform_jolt_constraints [ZKField f] (jolt_inputs : JoltR1CSInputs f) : ZKBuilder f PUnit := do
  ...
  ZKBuilder.constrainR1CS
    ...
    (jolt_inputs.RamAddress + (-1*jolt_inputs.Rs1Value) + (-1*jolt_inputs.Imm))
    0
  -- RamAddrEqZeroIfNotLoadStore
  ZKBuilder.constrainR1CS
    (1 + (-1*jolt_inputs.OpFlags_Load) + (-1*jolt_inputs.OpFlags_Store))
  ...
  ZKBuilder.constrainR1CS
    ...
    jolt_inputs.LeftLookupOperand
    0
  -- LeftLookupEqLeftInputOtherwise
  ZKBuilder.constrainR1CS
    (1 + (-1*jolt_inputs.OpFlags_AddOperands) + (-1*jolt_inputs.OpFlags_SubtractOperands) + (-1*jolt_inputs.OpFlags_MultiplyOperands))
  ...
  ZKBuilder.constrainR1CS
    jolt_inputs.OpFlags_VirtualInstruction
    (-1 + jolt_inputs.NextPC + (-1*jolt_inputs.PC))
    0
  -- MustStartSequenceFromBeginning
  ZKBuilder.constrainR1CS
    (jolt_inputs.NextIsVirtual + (-1*jolt_inputs.NextIsFirstInSequence))
    (-1 + jolt_inputs.OpFlags_DoNotUpdateUnexpandedPC)
    0
